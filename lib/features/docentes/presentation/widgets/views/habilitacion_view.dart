...
class _HabilitacionViewState extends State<HabilitacionView> {
          ...
          _FilterChip(
            ...
            onTap: () => showDialog(
              ...
              builder: (ctx) => SimpleDialog(
                ...
                children: [
                  _dialogOpt(ctx, 'Activas',   () { setState(() => _filtroActivo = true);  _load(); }),
                  _dialogOpt(ctx, 'Inactivas', () { setState(() => _filtroActivo = false); _load(); }),
                  _dialogOpt(ctx, 'Todas',     () { setState(() => _filtroActivo = null);  _load(); }),
                ],
              ),
            ),
          ),
          ...
          _FilterChip(
            ...
            onTap: () => showDialog(
              ...
              builder: (ctx) => SimpleDialog(
                ...
                children: [
                  _dialogOpt(ctx, 'Todos',      () { setState(() => _filtroNivel = null);         _load(); }),
                  _dialogOpt(ctx, 'Módulos',    () { setState(() => _filtroNivel = 'MODULO');     _load(); }),
                  _dialogOpt(ctx, 'Asignaturas',() { setState(() => _filtroNivel = 'ASIGNATURA'); _load(); }),
                ],
              ),
            ),
          ),
  ...
  void _showEditSheet(HabilitacionEntity hab) {
    showModalBottomSheet(
      ...
      builder: (_) => _EditHabilitacionSheet(
        habilitacion: hab,
        onSaved:      _load,
      ),
    );
  ...
  void _showCreateSheet() {
    showModalBottomSheet(
      ...
      shape: const RoundedRectangleBorder(
          borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
      builder: (_) => _CreateHabilitacionSheet(
        docenteId: widget.docenteId,
        onSaved:   _load,
      ),
    );
            ...
            Wrap(
              ...
              children: [
                ...
                if (h.fechaHasta != null)
                  _SmallBadge(
                    label: 'Hasta ${h.fechaHasta!.day}/${h.fechaHasta!.month}/${h.fechaHasta!.year}',
                    color: Colors.orange,
        ...
        children: [
          Row(children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(h.moduloNombre ?? h.asignaturaNombre ?? '',
          ...
          InkWell(
            ...
            child: Container(
              ...
              child: Row(children: [
                const Icon(Icons.event_outlined, color: AppTheme.primary, size: 20),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Fecha hasta (opcional)',
                          style: TextStyle(fontSize: 11,
                              color: AppTheme.textSecondary.withOpacity(0.7))),
                      const SizedBox(height: 2),
                      Text(
                        _fechaHasta != null ? _dateStr(_fechaHasta!) : 'Indefinida',
                        style: TextStyle(
                          fontSize:   14,
                          fontWeight: FontWeight.w600,
...
class _CreateHabilitacionSheetState extends State<_CreateHabilitacionSheet> {
  ...
  Future<void> _save() async {
    ...
    if (_fechaDesde == null) { _showSnack('Selecciona la fecha de inicio'); return; }

    final docenteId = int.tryParse(_docenteCtrl.text.trim());
    final targetId  = int.tryParse(_targetCtrl.text.trim());

    if (docenteId == null) { _showSnack('ID de docente inválido'); return; }
    if (targetId  == null) {
      _showSnack('ID de ${_nivel == 'MODULO' ? 'módulo' : 'asignatura'} inválido');
      return;
    }
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: _nivelesOpciones.map((n) => DropdownMenuItem(
                  value: n.value, child: Text(n.label),
                )).toList(),
                onChanged: (v) => setState(() { _nivel = v!; _targetCtrl.clear(); }),
              ),
              const SizedBox(height: 16),
              TextFormField(
                ...
                keyboardType: TextInputType.number,
                decoration: InputDecoration(
                  labelText:  'ID ${_nivel == 'MODULO' ? 'Módulo' : 'Asignatura'}',
                  prefixIcon: Icon(_nivel == 'MODULO'
                      ? Icons.library_books_outlined
                      : Icons.menu_book_outlined),
                  helperText: 'Ingresa el ID del '
              ...
              Row(children: [
                Expanded(child: _DateTile(
                  label: 'Fecha desde *', date: _fechaDesde,
                  required: true, onTap: () => _pickDate(true),
                )),
                const SizedBox(width: 12),
                Expanded(child: _DateTile(
                  label: 'Fecha hasta', date: _fechaHasta,
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _saving
                      ...
                      : const Text('Guardar habilitación',
                          style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
                ),
              ),
            ],
